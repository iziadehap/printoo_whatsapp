...
// ─────────────────────────────────────────────────────────────────────────────
// Core Business Logic (MOCKED for printing, REAL for configuration and caching)
// ─────────────────────────────────────────────────────────────────────────────
...
function detectPrinters() {
  return Promise.resolve([
    'Mock HP LaserJet Professional (Office)',
    'Mock Epson EcoTank L3250 (Color)',
    'Mock Canon PIXMA G3010'
  ]);
}
...
async function getPdfPageCount(filepath) {
  ...
  return 1; // mock default if parsing fails
}
...
async function printImage(filepath, printer, copies = 1) {
  printLog.info(`[🖨️ MOCK PRINT IMAGE] Spooling image: ${path.basename(filepath)} to printer: "${printer}" (copies: ${copies})`);
  return Promise.resolve();
}
...
function printDocumentWithGhostscript(filepath, printer, copies = 1, duplex = 'simplex') {
  printLog.info(`[🖨️ MOCK PRINT DOC] Spooling document: ${path.basename(filepath)} to printer: "${printer}" (copies: ${copies}, duplex: ${duplex})`);
  return Promise.resolve();
}
...
app.post('/api/print', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Jobs spooled successfully (Mocked)' });

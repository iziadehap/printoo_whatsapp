...
class StatusNotifier extends StateNotifier<AppStatus> {
  ...
  StatusNotifier(this._repository) : super(const AppStatus()) {
    _startPolling();
  }
  ...
  void _startPolling() {
    _timer = Timer.periodic(const Duration(seconds: 2), (_) async {
      if (!state.whatsappConnected) {
        try {
          debugPrint('[Status] Polling status...');
...
final statusProvider = StateNotifierProvider<StatusNotifier, AppStatus>((ref) {
  ...
  return StatusNotifier(repository);
});
...
class PrintersNotifier extends StateNotifier<List<String>> {
  ...
  PrintersNotifier(this._repository) : super([]) {
    _load();
  }

  Future<void> _load() async {
    try {
      state = await _repository.getPrinters();
      debugPrint(
...
final printersProvider = StateNotifierProvider<PrintersNotifier, List<String>>((
  ...
  final repository = ref.watch(appRepositoryProvider);
  return PrintersNotifier(repository);
});
...
final daysLookbackProvider = StateProvider<dynamic>((ref) => 0);
